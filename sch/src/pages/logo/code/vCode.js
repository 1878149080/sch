 ...
 * @url  https://github.com/saucxs/captcha-mini/blob/master/captcha-mini.js
 */
 ...
 * @desc 获取当前验证码的值
 * */
CaptchaMini.prototype.getCode = function () {
  return this.displayText || 'ones';
};
...
CaptchaMini.prototype.font = function () {
  ...
  for (let i = 0; i < this.length; i++) {
    ...
    this.paint[colorStyle] = '#fff';
    /*开始绘制*/
    this.paint.save();
    this.paint.rotate((deg * Math.PI) / 180);
    this.paint[fontStyle](str[i], x, this.canvas.height / 1.75);
    this.paint.restore();
  }
};
...
CaptchaMini.prototype.draw = function (dom, callback = function () {}) {
  ...
  // let colors = this.getColor(this.backGroundColor);
  this.paint.fillStyle = 'rgba(255,255,255,0.3)';
